...
namespace TGS.Challenge.Tests
{
    ...
    public class FormatNumberTests
    {
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => _formatNumber.Format(1000000000));
        }
        ...
        [Test()]
        public void Value_35235235_Returns_ValidString_Dot_Separator()
        {
            var formatNumber = new FormatNumber(".");
            var formatted = formatNumber.Format(35235235);

            Assert.AreEqual("35.235.235", formatted);
        }

        [Test()]
        public void Value_35235235_Returns_ValidString_Interval_Four()
        {
            var formatNumber = new FormatNumber(separatorInterval: 4);
            var formatted = formatNumber.Format(35235235);

            Assert.AreEqual("3523,5235", formatted);
        }
    }
}